...
test("login function, should return a 200 status code", async ()=>{
    let user = {email:"cipriano.alvarez223@gmail.com",password:"Stanleytobar3!",secret:process.env.APP_SECRET};
...
test("login function should return an error code 403 for invalid secret",async  ()=>{
    let user = {email:"cipriano.alvarez223@gmail.com",password:"Stanleytobar3!",secret:"12345"};
    let response = await request(app).post("/auth/login").send(user);
    expect(response.statusCode).toBe(403)
})
test("login function should return an error code 400 for no email",async  ()=>{
    let user = {password:"Stanleytobar3!",secret:process.env.APP_SECRET};
    let response = await request(app).post("/auth/login").send(user);
    expect(response.statusCode).toBe(400)
})
test("login function should return an error code 400 for no password",async  ()=>{
    let user = {email:"cipriano.alvarez223@gmail.com",secret:process.env.APP_SECRET};
    let response = await request(app).post("/auth/login").send(user);
    expect(response.statusCode).toBe(400)
})
test("login function should return an error code 404 for user not found",async  ()=>{
    let user = {email:"cipriano.alvarez3@gmail.com",password:"Stanleytobar3!",secret:process.env.APP_SECRET};
    let response = await request(app).post("/auth/login").send(user);
    expect(response.statusCode).toBe(404)
})
test("login function should return an error code 401 for invalid password",async  ()=>{
    let user = {email:"cipriano.alvarez223@gmail.com",password:"password123!",secret:process.env.APP_SECRET};
    let response = await request(app).post("/auth/login").send(user);
